...
RedisPipe.prototype._encodePayload = function( payload, ignoreMeta){
  var _payload = ignoreMeta? payload : {
    created: new Date(),
    payload: payload
  }
...
RedisPipe.prototype.ack = function(queueName, payload, callback){
  this._pub.lrem(this._processingQueue(queueName), -1, this._encodePayload(payload, true), callback);
}
