...
Notices.prototype.requeueDuration = function(queueName, duration, callback){
  callback = callback || function(){};
  if(this._pipe ) this._pipe.requeueDuration(this._queue(queueName), duration, callback);
}
...
Notices.prototype._queueify = function(obj){
  ...
  if(obj.requeue) throw new Error('Object already has a requeue method');
  obj.requeue = function(){
    self.requeue.apply(self, arguments);
  }

  if(obj.requeueDuration) throw new Error('Object already has a requeueDuration method');
  obj.requeueDuration = function(){
    self.requeueDuration.apply(self, arguments);
  }
